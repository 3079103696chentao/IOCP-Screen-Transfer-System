...
{
	m_isClosed = false;
	m_watchFramePending.store(false);
...
void CClientController::threadWatchScreen()
{
	...
	while (!m_isClosed) {
		if (!m_watchFramePending.load()) {
			const ULONGLONG now = GetTickCount64();
			if (now - nTick < 120) {
				Sleep(120 - DWORD(now - nTick));
			}
			nTick = GetTickCount64();
			const int ret = SendCommandPacket(m_watchDlg.GetSafeHwnd(), 6, true, NULL, 0);
			if (ret == 1) {
				m_watchFramePending.store(true);
			}
			else {
				TRACE("获取图片失败！ret = %d\r\n", ret);
			}
